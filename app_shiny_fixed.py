    ...
    ui.navset_tab(
        ...
        ui.nav(
            ...
            ui.row(
                ui.column(
                    ...
                    ui.input_select("new_status", "Status *", choices=["Cold", "Warm", "Hot"], selected="Warm"),
        ...
        ui.nav(
            ...
            ui.row(
                ...
                ui.column(6,
                    ui.h3("Conversion Metrics"),
                    ui.output_ui("conversion_metrics"),
                ),
            ),
            ui.br(),
...
def server(input, output, session):
    ...
    @render.text
    def hot_leads():
        return str(len(leads_data()[leads_data()['status'] == 'Hot']))
    ...
    def upcoming_table():
        ...
        if len(upcoming) > 0:
            ...
            return upcoming[display_cols].sort_values('next_followup')
        else:
            return pd.DataFrame(columns=['name', 'email', 'phone', 'status', 'next_followup', 'notes'])
    ...
    def _():
        df = leads_data()
        if len(df) > 0:
            status_choices = sorted(df['status'].unique().tolist())
            source_choices = sorted(df['source'].unique().tolist())

            ui.update_select("filter_status", choices=status_choices)
            ui.update_select("filter_source", choices=source_choices)
    ...
    def age_analysis():
        ...
        fig = px.scatter(
            ...
            title='Lead Age vs Quality (Bubble size = Quality)'
        )
    ...
    def source_performance():
        ...
        return source_analysis.sort_values('Hot %', ascending=False)
